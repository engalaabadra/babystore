  ...
  <div>
    ...
    <v-col cols="12" class="pb-3">
      <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
        ...
        <template v-slot:top>
          <v-toolbar flat color="white"> سلة محذوفات الحركات</v-toolbar>
        </template>
      </v-simple-table>
    </v-col>
    <template>
...
export default {
  ...
  methods: {
    ...
    getmovements() {
      this.$http
        .get(`admin/movements/trash?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    restoreItem(item) {
      ...
      this.$http
        .get(`admin/movements/restore/${this.editedItem.id}`)
        .then(res => {
    ...
    restoreAll() {
      this.$http
        .get('admin/movements/restore-all')
        .then(res => {
