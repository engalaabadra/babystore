<template>
  <div>
    ...
    <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
      ...
      <template v-slot:top>
        <v-toolbar flat color="white">سلة محذوفات ادارة  الدول</v-toolbar>
      </template>
    </v-simple-table>
    </v-col>
    <template>
      <v-pagination v-model="page" :length="pageInfo && pageInfo.last_page" @input="getcountries()" circle></v-pagination>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getcountries() {
      this.$http
        .get(`admin/countries/trash?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    restoreAll() {
      this.$http
        .get('admin/countries/restore-all')
        .then(res => {
