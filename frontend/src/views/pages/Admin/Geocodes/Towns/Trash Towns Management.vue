<template>
  <div>
    ...
    <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
      ...
      <template v-slot:top>
        <v-toolbar flat color="white">سلة محذوفات ادارة  المناطق</v-toolbar>
      </template>
    </v-simple-table>
    </v-col>
    <template>
...
export default {
  ...
  methods: {
    ...
    gettowns() {
      this.$http
        .get(`admin/towns/trash?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    restoreItem(item) {
      ...
      this.$http
        .get(`admin/towns/restore/${this.editedItem.id}`)
        .then(res => {
