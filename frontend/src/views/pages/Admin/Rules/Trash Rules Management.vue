  ...
  <div>
    ...
    <v-col cols="12" class="pb-3">
      <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
        ...
        <template v-slot:top>
          <v-toolbar flat color="white"> سلة محذوفات ادارة قوانين النظام</v-toolbar>
        </template>
      </v-simple-table>
    </v-col>
    <template>
...
export default {
  ...
  methods: {
    ...
    getrules() {
      this.$http
        .get(`admin/rules/trash?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    restoreItem(item) {
      ...
      this.$http
        .get(`admin/rules/restore/${this.editedItem.id}`)
        .then(res => {
