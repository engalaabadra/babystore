      ...
      <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
        ...
        <template v-slot:top>
          <v-toolbar flat color="white">
            ادارة الرسائل المرسلة من الاعضاء الي(استقبلهم)
                  ...
                  <v-card class="col-sm-7 mx-auto">
                    <v-card-title>
                      <v-alert class="col-sm-12 mx-auto white--text font-2 text-center" color="primary">
                        <v-icon large>mdi-account-circle</v-icon> ادارة الرسائل المرسلة من الاعضاء الي(استقبلهم)
                      </v-alert>
                    </v-card-title>
                    <v-card-text>
...
export default {
  ...
  methods: {
    ...
    showTrash() {
      this.$router.push('/trash-chats-recived-management')
    },

    getchats() {
      this.$http
        .get(`admin/chats/get-all-chats-recived-paginates?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    save() {
      this.$http
        .post('admin/chats/store', {
          ...
          user_id: 1,
          client_id: this.editedItem.user.id,
          chat_room_id: 1,
        })
