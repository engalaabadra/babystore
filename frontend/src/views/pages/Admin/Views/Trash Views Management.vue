  ...
  <div>
    ...
    <v-col cols="12" class="pb-3">
      <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
        ...
        <template v-slot:top>
          <v-toolbar flat color="white"> سلة محذوفات المشاهدات</v-toolbar>
        </template>
      </v-simple-table>
    </v-col>
    <template>
...
export default {
  ...
  methods: {
    ...
    getviews() {
      this.$http
        .get(`admin/views/trash?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    restoreItem(item) {
      ...
      this.$http
        .get(`admin/views/restore/${this.editedItem.id}`)
        .then(res => {
    ...
    deleteItem(item) {
      ...
      confirm('هل أنت متأكد من حذف هذا العنصر؟') &&
        this.$http
          .get(`admin/views/force-delete/${item.id}`)
          .then(res => {
            this.views.splice(index, 1)
            this.$store.state.snackbar = true
            this.$store.state.text = res.data.message
