<template>
  <div>
    ...
    <v-simple-table class="mx-auto pb-5 rounded-xl elevation-10">
      ...
      <template v-slot:top>
        <v-toolbar flat color="white"> سلة محذوفات ادارة وسائل الدفع</v-toolbar>
      </template>
    </v-simple-table>
    <template>
      <v-pagination
...
export default {
  ...
  methods: {
    ...
    getpayments() {
      this.$http
        .get(`admin/payments/trash?page=${this.page}&total=${this.total}`)
        .then(res => {
    ...
    restoreItem(item) {
      ...
      this.$http
        .get(`admin/payments/restore/${this.editedItem.id}`)
        .then(res => {
    ...
    restoreAll() {
      this.$http
        .get('admin/payments/restore-all')
        .then(res => {
          if (res.data.message != null) {
            this.payments = []
            this.$store.state.snackbar = true
